...
- An `events` array whose entries have a unique event `type`, readable label, concise description, and optional `payload: { key, label, options: [{ label, value }] }`. Options are a finite set of distinct JSON scalars (string, finite number, boolean, or null). Emission supplies exactly `{ [key]: selectedValue }`; an event without a payload schema accepts only an omitted payload. Event types use stable source-derived names such as `SOURCE_VALID`.
- A `transitions` array whose entries have a unique stable `id`, valid `from` and `to` state IDs, an event type defined by `events`, optional `guard` and scalar `payload` match, zero or more declarative context updates, zero or more intended side-effect descriptions, and zero or more source references. An omitted transition `payload` matches any allowed option for that event.
...
Keep dispatch declarative. A guard is `{ source: "context" | "payload", key, op: "equals" | "not-equals", value }`, comparing one existing field with a JSON scalar by strict equality or inequality; missing fields do not match. Updates are `{ op, key, value? }`: literal `set`, numeric `increment`/`decrement`, or array `append`/`remove-last`. Numeric amounts are finite numbers; only an omitted amount defaults to one, and explicit zero remains zero. If a condition cannot be represented faithfully, expose distinct source-feasible events or finite symbolic choices. Never turn target text into executable JavaScript.

For a source condition represented by a named symbolic event rather than an evaluated comparison, retain its explanation in `guardDescription`, not `guard`. The renderer labels it as a symbolic event condition; it is not execution evidence.
...
- Emitting an event with zero or multiple matching transitions leaves state and context unchanged and briefly marks rejection within the graph canvas. Ambiguous models must be repaired before publication, never resolved by choosing the first edge.
